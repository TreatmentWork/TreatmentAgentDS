...
function NodeJSScanner(options) {
    ...
    if (options.hasOwnProperty('gpg') && Object.keys(options.gpg).length > 0) {
        this.settings.gpg = __.extend({},this.settings.gpg, options.gpg);
        delete options.gpg;
    }
...
NodeJSScanner.prototype.is_infected = function(file, callback) {
    ...
    var dsfiles = [];
    dsfiles.push(file + '.ds');
    dsfiles.push(file);
    console.log('Nodejs-ClamAv-Scanner: Configured clam command: ' + this.settings[this.scanner].path + ' ' + this.build_clam_args(dsfiles).join(' '));


    execFile(this.settings[this.scanner].path, this.build_clam_args(dsfiles), function(err, stdout, stderr) {
    ...
    if(stdout) {
      ...
      result = stderr.trim();
    	...
    	var goodbad = "Good signature";
    	console.log("result: " + result);
    	if(result.indexOf(goodbad) != -1) {
    		console.log("Nodejs-ClamAv-Scanner indexOf: " + file + ' is VALID!');
        callback(null, file, true);
    	} else {
    		console.log("Nodejs-ClamAv-Scanner indexOf: " + file + ' is INVALID!');
